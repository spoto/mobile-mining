...
from Google Play~\cite{mokaminter-google-play}. Its source code is available from~\cite{mokaminter-source-code}. An iPhone version migt be available
in the future, but it is more complex to create, since it is not possible
to leverage the same Java libraries used for a desktop miner.
...
\item the app creates a new private/public key pair to identify the miner,
  ...
  in terms of a $12$ words mnemonics;
  \begin{itemize}
...
\item once the plot file is created, the app activates a background process
  that mines by using the new plot file.
\end{enumerate}
