...
\newcommand{\phone}{\includegraphics[width=1.3cm]{pictures/phone}}
...
\begin{frame}{Blockchain mining as a competitive puzzle}
  ...
  \item[{\color{red}answer:}] \only<3->{reply with a \textbf{next} that satisfies the challenge}
  \item[{\color{red}competition:}] \only<4->{in case of more \textbf{next}, select a \emph{best} one}
  \end{description}

  \bigskip
  \visible<5->{Bitcoin's Proof of Work:}
  \begin{description}
  \item<6->[{\color{red}challenge:}] find a \textbf{next} such that $\hash(\mathbf{next})>\difficulty$
  \item<7->[{\color{red}answer:}] reply with a \textbf{next} obtained
...
\begin{frame}\frametitle{Miners as mobile applications}
  ...
  \item it simplifies the start-up of new blockchains, since it is easy to convince people
    to install an app
  \end{itemize}
...
\begin{frame}\frametitle{Miners as mobile applications}
  ...
  \item no, since blocks are too large for being streamed to a mobile
    \begin{itemize}
    \item mobile networks are still too slow, especially in less fortunate countries
...
\begin{frame}\frametitle{Miners as mobile applications, again}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1]
      ...
      \draw[->, thick] (3.5,1.0) -- (7,1.0);
      \draw (5.25,1.2) node {{\small challenges ($\langle i,\gamma\rangle$)}};
...
\begin{frame}\frametitle{Experiments: mining phase}
  ...
  In the remaining $\mathit{blockrate}-0.225$ seconds the miner is doing \alert{nothing}:
  \begin{itemize}
