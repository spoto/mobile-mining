...
Mokamint's idea of a generic blockchain engine with
a pluggable application layer has been borrowed from Tendermint~\cite{Kwon14},
...
\item the user specifies the URI (universal resource identifier) of a
  blockchain peer running Mokamint and the size of the plot file for the miner
...
\item the app creates a new private/public key pair to identify the miner,
  ...
  \item alternatively, the user can insert the $12$ words of a key pair,
    if she wants to use an existing one;
  \end{itemize}
...
\item if confirmed, the creation of the plot file runs in the background;
...
or steals the phone would be able to access and steal the cryptocurrency
accumulated while mining.
Because of this, it has been an explicit design choice for Mokaminter to avoid
storing the private key of the miners in the mobile phone.
Namely, after creating or importing the private/public key pair (Fig.~\ref{fig:mokaminter_add_miner}),
...
well as the logs of the running application. In a mobile app, this is possible
but not comfortable for the average user. Therefore, Mokaminter gives feedback through
...
Inserting words on a mobile phone, through a software keyboard, is frustrating
and error-prone.
Therefore, Mokaminter suggests the possible words while typing, so that
the user can autocomplete them.
%
A last issue is about the risk of specifying a plot file's size
that is too large to complete the creation of the plot in a reasonable time:
it would hang the mobile phone and drain its battery. To avoid this risk,
...
mobile phone is unreliable in comparison to that of a desktop computer, since
it gets lost frequently (thick walls, tunnels, network switches).
Mokaminter automatically tries to reconnect whenever the connection gets down or
...
phone productors often add extra constraints. On many phones, the Android system
turns off Mokaminter overnight, when
