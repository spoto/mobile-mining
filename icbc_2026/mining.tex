...
Its goal is to choose which peer wins the right to create and add the \textbf{next}
block to the stable chain of the blockchain. This block must be \emph{valid}, that is, respect
...
actual list of consensus rules is quite larger and is not the topic of this paper.

Mining algorithms can be presented as a game that works in terms of:
%
\begin{itemize}
\item \emph{challenge}: given the current chain $\xi$,
  determine, deterministically, a challenge for \textbf{next};
...
\end{itemize}
%
Each peer performs the mining algorithm concurrently and independetly from the other peers,
...
in terms of the \emph{smallest} hash, but we preferred the equivalent
definition above, given in terms of the largest hash,
...
\end{itemize}
%
The algorithm for computing $i$ is irrelevat here: often, a simple
...
set of precomputed nonces on disk (called, a \emph{plot file}). The mining algorithm in this case
becomes:
%
\begin{itemize}
...
is that the computation of the nonces (\emph{i.e.}, of the plot file) is so expensive
...
challenges. The consensus algorithm will use $\delay$ to wait until
the moment when the block becomes valid.
...
This is shown in Fig.~\ref{fig:local_mining}, where the peer that creates the block
and the miner that provides the answer to create the block are two distinct software
