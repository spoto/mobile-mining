...
of a desktop computer using a reliable home internet connection.
The exact earnings may vary
randomly in different runs, for the probabilistic
nature of the mining algorithm, but their order of magnitude is similar.
Note that earnings cannot be expressed in fiat
currency, since Hotmoka's mokas are not currently exchangeable.
...
These mining experiments have been performed on the Hotmoka mainnet, that creates a block every $20$ seconds,
on average. Therefore, column \textbf{answer} in Fig.~\ref{fig:mining_phase_experiments} tells that
Mokaminter sits idle for about $99\%$ of the time, waiting at step~\ref{step:deadline_mining:wait}
...
An exact evaluation of the battery use is difficult since it depends on the other apps
installed in the device and on the usage pattern of the device by its user.
In order to get some realistic hint, we have installed Mokaminter in the phone of
a group of $11$ professors and students from the UFMA university in Brazil
(other users owned only an iPhone and could not take part in the experiment).
Each of them created a miner and let it run in the phone for a day. At the end,
the settings page for Mokaminter in Android reported the percent of battery used.
Mokaminter consumed only $0.27\%$ of battery power, on average.
...
Mokaminter has also been used during
