...
\begin{enumerate}
  \item The exact notion of transaction. This is irrelevant in this paper.
    ...
    For instance, a transaction is a money transfer
    in Bitcoin~\cite{Nakamoto08}, while in Ethereum~\cite{AntonopoulosW25}
  ...
  \item The chosen mining algorithm. Namely, Bitcoin pioneered the
    proof of work algorihtm~\cite{DworkN92}, while more modern blockchains tend to use a
    proof of stake algorithm~\cite{Kwon14}, and this paper focuses on a specific proof
    of space algorithm~\cite{Spoto25}.
\end{enumerate}

In Bitcoin's proof of work,
minted blocks must carry evidence of some computational
...
It is well-known how this has led to high mining costs, in terms of electricity for CPU
computation and data centers, and consequently to high ecological costs (resource
consumption and e-waste). Moreover, this has led to the concentration of mining power in countries
...
it is generally accepted that proof of stake remains more centralized than proof of work.
...
Its idea is that minted blocks must carry evidence
of the allocation of a large data structure on disk. The larger this data structure,
...
\item it makes mining available to a larger portion of the humanity, who owns a mobile
...
in least favorite countries. As this paper will show, this limit depends on the
specific mining algorithm. Namely, proof of space emerges here as a niche solution that
suffers much less from the limitations of mobile mining.

This paper makes the following contributions:
%
\begin{enumerate}
\item it presents mobile mining, its power and its limitations;
...
\item it reports experimental results with Mokaminter, discussing the battery consumption
  and the amount of exchanged data.
\end{enumerate}
%
We stress that what we will let run in the phone is the miner only, not a full blockchain
peer, which would be prohibitive for many reasons, such as the size of the blockchain state
in the database of the peer, the amount of the exchanged data and the computational
cost. The blockchain peer is assumed to be running on some computer
reachable online from the miner running in the phone.

The rest of this paper is organized as follows.
Sec.~\ref{sec:related_work} discusses related work.
Sec.~\ref{sec:mining} defines mining in a uniform setting that encompasses
various existing algorithms.
Sec.~\ref{sec:remote_mining} considers the interaction between the
chosen mining algorith and the possibility of implementing it with
a miner running on a mobile phone, connected to a blockchain peer.
Sec.~\ref{sec:mokaminter} presents the Mokaminter app for mining on
a mobile phone with proof of space.
Sec.~\ref{sec:experimental_results} reports experiments to measure
exchanged data and battery comsumption due to Mokaminter.
Sec.~\ref{sec:conclusion} draws the conclusions.
