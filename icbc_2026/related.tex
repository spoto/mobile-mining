...
The probability that a miner creates a next block is proportional
to the disk space allocated during the plotting phase.

Pioneering works~\cite{AtenieseBFG14,DziembowskiFKP15} introduce
formal models of proofs baced on persistent storage, showing that
...
\subsection{Mobile Apps for Mining on a Phone}
...
tasks to external servers and the cloud~\cite{SoundararajS25}.
But this is not what this paper does,
which is rather to actually perform mining \emph{in} the mobile phone
and to offload the mining process from the blockchain peers to the many
mobile phones that are expected to run the mobile mining app.
...
despite their description, are not miners of cryptocurrencies, but rather:
