...
\begin{figure}[t]
  \begin{center}
    \begin{tikzpicture}
      \draw [fill=verylightviolet,thick] (-0.5,-0.3) rectangle +(4,4.7);
      \draw (1.5,2.5) node {blockchain peer};
      \draw (1.5,1.9) node {{\small (networking, consensus}};
      \draw (1.5,1.5) node {{\small and application)}};

      \draw [fill=verylightred,thick] (7,3) rectangle +(2,0.8);
      \draw (8.0,3.4) node {$\text{miner}_1$};
      \draw[->, thick] (3.5,3.65) -- (7,3.65);
      \draw (5.25,3.85) node {{\small challenges}};
      \draw[<-, thick] (3.5,3.15) -- (7,3.15);
      \draw (5.25,2.9) node {{\small answers}};
      \draw (5.25,2.65) node {{\scriptsize (in this paper, nonces)}};
      ...
      \draw (5.25,0) node {{\scriptsize (in this paper, nonces)}};
    \end{tikzpicture}
  \end{center}
