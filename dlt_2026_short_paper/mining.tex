...
goal to decide which peer may add a \textbf{next} block to the current chain $\xi$:
%
\begin{itemize}
\item \emph{challenge}: compute, deterministically,
  from $\xi$ a challenge for \textbf{next};
...
whose challenge is the index $i$ of the chosen validator,
...
%
\begin{itemize}
\item \emph{challenge}: compute a data structure from $\xi$ (see Sec.~\ref{sec:pos});
\item \emph{answer}: provide one of the nonces in the precomputed plot,
  the one that minimizes its \emph{waiting time} \wrt{} the challenge;
...
is that the computation of the plot is so expensive
that it cannot be done on-the-fly, for each mined block, but rather
once and for all, before starting mining.
The plot remains on disk to answer, quickly, mining challenges.
...
are logically separate software parts of the same tool. Miners do not store
...
Up to now, miners run on specialized, very expensive hardware. The goal of this article
...
the mining algorithm is one of those discussed in Sec.~\ref{sec:introduction}.
